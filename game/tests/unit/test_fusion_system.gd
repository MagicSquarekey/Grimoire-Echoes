...
func test_fusion_recipe_count() -> void:
	var recipes = FusionSystem.FUSION_RECIPES
	assert(recipes.size() == 15, "融合配方数量应为15，实际: %d" % recipes.size())
	print("✅ 融合配方数量测试通过")
...
func test_fusion_recipe_uniqueness() -> void:
	...
	for recipe in recipes:
		assert(recipe.id not in ids, "融合配方ID重复: " + recipe.id)
...
func test_fusion_recipe_elements() -> void:
	...
	for i in range(recipes.size()):
		...
		assert(recipe.element1 == expected[0] and recipe.element2 == expected[1],
...
func test_fusion_recipe_damage() -> void:
	var recipes = FusionSystem.FUSION_RECIPES
	for recipe in recipes:
		assert(recipe.base_damage >= 0.0, "配方 %s 伤害值无效: %f" % [recipe.id, recipe.base_damage])
	print("✅ 融合配方伤害值测试通过")
...
func test_fusion_availability() -> void:
	...
	assert(available.size() == 0, "空法术数组应无可用融合")
	print("✅ 融合条件检查测试通过")
